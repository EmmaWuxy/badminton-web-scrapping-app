...
def create(headings:list, table_array:list, num_display:Optional[int], excel:bool)->None:
    ...
    [sg.Text(key='EXCEL')]]
    table_window = sg.Window("Badminton Center Information Window", table_layout, size = (1800,600))
    ...
    if excel:
        table_window['EXCEL'].update('The talbe is also stored in {}\output.xsl'.format(os.getcwd()))
